...
describe("FrameSelector", () => {
  it("matches every proven 12 m baseline frame field", async () => {
    const result = selectFrame({ span_m: 12, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: null, climate }, await frame(12));
  ...
  it.each([9, 15, 18, 21] as const)("returns a deterministic implemented result for %d m", async (span) => {
    ...
    const input = { span_m: span, building_height_m: 3, responsibility_factor: 0.8 as const, frame_step_override_m: null, climate };
  ...
  it("preserves 24 m legacy #N/A instead of selecting a corrected profile", async () => {
    const result = selectFrame({ span_m: 24, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: null, climate }, await frame(24));
  ...
  it("supports an exact proven manual frame step", async () => {
    const result = selectFrame({ span_m: 12, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: 6, climate }, await frame(12));
    expect(result.status).toBe("success");
  ...
  it("keeps the opposite reliability block mapping for responsibility 1.0", async () => {
    const result = selectFrame({ span_m: 12, building_height_m: 3, responsibility_factor: 1.0, frame_step_override_m: null, climate }, await frame(12));
    expect(result.status).toBe("success");
  ...
  it("returns UNKNOWN_FRAME_DOMAIN for a manual step absent from the table", async () => {
    const result = selectFrame({ span_m: 12, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: 5, climate }, await frame(12));
  ...
  it("returns FRAME_NO_MATCH for a climate branch not present in the selected table", async () => {
    const result = selectFrame({ span_m: 12, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: null, climate: { ...climate, snow_region: "IV", wind_region: "II" } }, await frame(12));
  ...
  it("returns INVALID_FRAME_INPUT for a non-positive building height", async () => {
    const result = selectFrame({ span_m: 12, building_height_m: 0, responsibility_factor: 0.8, frame_step_override_m: null, climate }, await frame(12));
  ...
  it("does not contaminate a 9 m selection with the 12 m dataset", async () => {
    const nine = selectFrame({ span_m: 9, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: null, climate }, await frame(9));
    const twelve = selectFrame({ span_m: 12, building_height_m: 3, responsibility_factor: 0.8, frame_step_override_m: null, climate }, await frame(12));
