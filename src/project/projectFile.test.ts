...
describe("Sprint-M project file", () => {
  ...
  it("restores the complete versioned ProjectInput and ignores derived result fields", () => {
    const result = parseSprintMProjectFile(JSON.stringify({ format: "SPRINT_M_PROJECT", version: 1, savedAt: "2026-09-20T10:00:00Z", project, result: { D69: 999999, selectedProfile: "FAKE" } }));
  ...
  it("rejects wrong format and unsupported versions", () => {
    ...
    expect(parseSprintMProjectFile(JSON.stringify({ format: "SPRINT_M_PROJECT", version: 2, project }))).toEqual({ ok: false, message: "Версия файла расчёта не поддерживается: файл v2, приложение поддерживает v1." });
  });
