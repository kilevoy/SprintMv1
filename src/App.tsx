...
function ResultCard({ result, onManualClimate, climateFresh = true }: { result: Core1EngineResult | null; onManualClimate: () => void; climateFresh?: boolean }) {
  ...
  const frameCount = typeof value.scenario.building_length_m === "number" && typeof value.frame_step_m === "number" && value.frame_step_m > 0
    ? Math.ceil(value.scenario.building_length_m / value.frame_step_m) + 1
    : null;
  const limited24m = value.scenario.span_m === 24;
  return <div className="result-stack">
    ...
    {limited24m ? <div className="diagnostics domain-notice" role="status"><details open><summary><strong>CORE1_24M_STRUCTURAL_PARITY_PARTIAL</strong><span>24 м: подбор рамы и автоматический шаг подтверждены; итоговый структурный показатель D69 имеет частично восстановленную legacy-ветку.</span></summary><div className="diagnostic-details"><span>Числа не изменяются; верхние высоты, ручной D9, ROW15 и часть connection-ветки остаются вне доказанного домена.</span></div></details></div> : null}
    <section className="result-card"><div className="card-heading"><div><span className="eyebrow">Core 1</span><h3>Основной каркас</h3></div><span className="tiny-status">Результат</span></div><div className="metric-grid"><Metric label="Балка" value={value.beam_profile} sub={`${value.beam_steel ?? "—"} · ${format(value.beam_utilization, 1)} %`} /><Metric label="Колонна" value={value.column_profile} sub={`${value.column_steel ?? "—"} · ${format(value.column_utilization, 1)} %`} /><Metric label="Эффективный шаг рам" value={`${format(value.frame_step_m)} м`} sub={value.scenario.frame_step_override_m == null ? "D8 · автоматически" : "D9 · ручной override"} /><Metric label="Количество рам" value={frameCount ?? "нет данных"} /><Metric label="Прогоны" value={value.purlin_profile} sub={`${value.scenario.roof_deck_grade ?? "настил —"} · шаг ${value.purlin_step_mm != null ? `${format(value.purlin_step_mm, 0)} мм` : "по расчёту"} · ${value.purlin_steel ?? "—"} · ${format(value.purlin_kg_per_m2)} кг/м²`} /></div></section>
    <section className="result-card"><div className="card-heading"><div><span className="eyebrow">Масса</span><h3>Проёмы и окна</h3></div></div><div className="metric-grid"><Metric label="Дополнительная масса" value={`${format(value.openings_weight_kg_per_m2)} кг/м²`} /><Metric label="Абсолютная масса" value={`${format(value.openings_weight_t, 3)} т`} /><Metric label="Оконные ригели" value={value.window_girts_weight_kg ? `${format(value.window_girts_weight_kg)} кг` : "нет"} /></div>{value.scenario.windows?.enabled ? <div className="window-result"><strong>Тип окна {value.scenario.windows.window_type}</strong><span>Нижний: {value.window_lower_girt_profile ?? "нет данных"} · {format(value.window_lower_girt_utilization, 1)} %</span><span>Верхний: {value.window_upper_girt_profile ?? "нет данных"} · {format(value.window_upper_girt_utilization, 1)} %</span><em>ENHANCED · локальный подбор оконных ригелей; LEGACY_COMPARABLE не доказан без golden-сценария.</em></div> : null}</section>
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><div className="intro"><div><span className="eyebrow">Инженерный калькулятор</span><h1>Подбор сечений</h1><p>Один ProjectInput для пользовательского проекта и будущих расчётных ядер.</p></div><StatusBadge result={result} /></div>
      ...
      <div className="calculator-grid"><form className="input-panel" onSubmit={(event) => { event.preventDefault(); void calculate(); }}>
        ...
        <Section title="Геометрия ангара"><div className="two-col"><Field label="Пролёт, м"><input aria-label="Пролёт, м" type="number" min="0.01" max="24" step="any" value={projectInput.geometry.span_m} onChange={(event) => updateGeometry({ span_m: numberValue(event.target.value) })} /><small className="field-note">Литературный пролёт сохраняется; Core 1 маршрутизирует его в доказанную семейную ветку.</small></Field><Field label="Ответственность"><select value={projectInput.geometry.responsibility_factor} onChange={(event) => updateGeometry({ responsibility_factor: Number(event.target.value) as 0.8 | 1.0 })}><option value="0.8">0,8</option><option value="1.0">1,0</option></select></Field></div><div className="two-col"><Field label="Длина, м"><input type="number" min="0.01" step="any" value={projectInput.geometry.building_length_m} onChange={(event) => updateGeometry({ building_length_m: numberValue(event.target.value) })} /></Field><Field label="Высота, м"><input type="number" min="0.01" max="6.2" step="any" value={projectInput.geometry.building_height_m} onChange={(event) => updateGeometry({ building_height_m: numberValue(event.target.value) })} /></Field></div><div className="frame-step-mode"><span className="field-label">Шаг рам</span><label className="check-row"><input type="radio" name="frame-step-mode" checked={projectInput.geometry.frame_step_override_m === null} onChange={() => updateGeometry({ frame_step_override_m: null })} /><span>Автоматически</span></label><small className="field-note">Расчётный шаг D8 определяется Core 1 после расчёта.</small><label className="check-row"><input type="radio" name="frame-step-mode" checked={projectInput.geometry.frame_step_override_m !== null} onChange={() => updateGeometry({ frame_step_override_m: 6 })} /><span>Задать вручную (D9)</span></label>{projectInput.geometry.frame_step_override_m !== null ? <Field label="Ручной шаг D9, м"><input type="number" min="0.01" step="any" value={projectInput.geometry.frame_step_override_m} onChange={(event) => updateGeometry({ frame_step_override_m: numberValue(event.target.value) })} /></Field> : null}</div></Section>
        <Section title="Ограждающие конструкции" hint="Проектная оболочка разделена с Core 1"><div className="construction-group"><div className="construction-kicker"><strong>Кровля</strong><span>Формирует вход Core 1</span></div><Field label="Покрытие"><select value={projectInput.envelope.roof_covering} onChange={(event) => updateEnvelope({ roof_covering: event.target.value as RoofCovering })}>{roofCoverings.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="Марка настила"><select value={projectInput.envelope.roof_deck_grade} onChange={(event) => updateEnvelope({ roof_deck_grade: event.target.value as RoofDeckGrade })}>{deckGrades.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><p className="field-help">Используется для ограничения допустимого шага прогонов.</p></div><div className="construction-group"><div className="construction-kicker"><strong>Стены · будущий EnclosureCore</strong><span>Проектный параметр, не расчёт полной стены в Core 1</span></div><Field label="Система стен"><select aria-label="Стены" value={projectInput.envelope.wall_system} onChange={(event) => updateEnvelope({ wall_system: event.target.value })}>{wallSystems.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field><p className="field-help muted">Материал, утепление и стеновые ригели относятся к будущему EnclosureCore; Core 1 не подменяет ими расчёт каркаса.</p></div></Section>
